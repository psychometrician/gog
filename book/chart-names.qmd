...
| Network diagram / node-link graph | ✅ drawn: `edge * layout(<from>, <to>) + network()`, with `point * layout(...)` for the nodes, `size(degree)` for their busyness, and `text * layout(...) + label(name)` for their names, worked through in [Network](network.qmd). The engine computes every position, so the picture is the same in all four languages; state an angle, `network(turn = 35, tilt = 20)`, and the same layout is a cube that drags in the web edition |
