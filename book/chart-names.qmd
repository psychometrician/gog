...
| Dendrogram / cluster tree | ✅ drawn: `path * cluster(<value>, over = <profile>) + x(<leaves>)` joins the closest leaves one pair at a time, with the merge distance on an axis of its own; `y(<leaves>)` lays it on its side ([Transforms](transforms.qmd#cluster-the-closest-leaves-joined-first)). *Dendrogram* is Greek for tree drawing, which is what the plain name says |
| Clustered heatmap | ✅ drawn: `zone * cluster(over = <profile>)` reorders a tile plot's slots to the tree's leaf order, and the full figure is the marginal plot, with trees above and beside the tiles; the shared axes take the trees' order ([Composition](composition.qmd#a-clustered-panel-decides-the-order)) |
