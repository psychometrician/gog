...
JL_COLUMN_ATOMS <- c("x", "y", "z", "color", "size", "shape", "opacity",
                     ...
                     "colour")
JL_ALL_COLUMN_ARGS <- c("bounds", "partition", "flow", "layout", "cluster")
  ...
  if (is.call(v)) {
    ...
    if (fn == "factor") {
      ...
      if (length(values) != 1 || length(levels) != 1 ||
        ...
        jl_note_gap("factor without written-out levels"); return(NA_character_)
      }
      v <- jl_value(values[[1]]); l <- jl_value(levels[[1]])
